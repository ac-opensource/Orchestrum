defmodule SymphonyElixir.Codex.AppServer do
        ...
        case await_turn_completion(port, on_message, tool_executor, auto_approve_requests) do
          {:ok, result} ->
            ...
            {:ok,
             %{
               result: result,
               port_exited: result == :port_exited_after_turn,
               session_id: session_id,
               thread_id: thread_id,
               turn_id: turn_id
             }}
  ...
  defp send_initialize(port) do
    ...
    with :ok <- send_message(port, payload),
         {:ok, _} <- await_response(port, @initialize_id) do
  ...
  defp start_thread(port, workspace, %{approval_policy: approval_policy, thread_sandbox: thread_sandbox}) do
    with :ok <-
         ...
         {:ok, %{"thread" => thread_payload}} <- await_response(port, @thread_start_id) do
      parse_thread_payload(thread_payload)
    end
  end

  defp parse_thread_payload(%{"id" => thread_id}), do: {:ok, thread_id}
  defp parse_thread_payload(thread_payload), do: {:error, {:invalid_thread_payload, thread_payload}}
  ...
  defp receive_loop(
    ...
    receive do
      ...
      {^port, {:exit_status, 0}} when turn_activity_seen? ->
        {:ok, :port_exited_after_turn}
      ...
      when is_binary(method) ->
        handle_turn_method(
          ...
          %{
            ...
            auto_approve_requests: auto_approve_requests,
            turn_activity_seen?: turn_activity_seen? or turn_activity_method?(method)
          }
        )
  ...
  defp handle_turn_method(
    ...
    on_message = loop_context.on_message
    timeout_ms = loop_context.timeout_ms
    tool_executor = loop_context.tool_executor
    auto_approve_requests = loop_context.auto_approve_requests
    turn_activity_seen? = Map.fetch!(loop_context, :turn_activity_seen?)
  ...
  defp turn_activity_method?("turn/started"), do: true
  defp turn_activity_method?("item/" <> _event), do: true
  defp turn_activity_method?("thread/tokenUsage/updated"), do: true
  defp turn_activity_method?(_method), do: false
