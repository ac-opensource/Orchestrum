defmodule SymphonyElixirWeb.Presenter do
  ...
  alias SymphonyElixir.{Config, Linear.Issue, LogFile, Orchestrator, ProjectConfig, StatusDashboard, Tracker}
    ...
    case Orchestrator.snapshot(orchestrator, snapshot_timeout_ms) do
      %{} = snapshot ->
        ...
        if is_nil(running) and is_nil(retry) and is_nil(task_issue) do
          ...
          {:ok, issue_payload_body(issue_identifier, running, retry, task_issue, snapshot)}
        end
  ...
  defp issue_payload_body(issue_identifier, running, retry, task_issue, snapshot) do
    ...
    %{
      ...
      tracked: tracked_payload(task_issue, source_control)
    }
  end
  ...
  defp tracked_payload(nil, source_control) do
    %{
  ...
  defp tracked_payload(task_issue, source_control) do
    Map.merge(task_issue, %{
      branch_name: source_control.branch_name || task_issue[:branch_name],
      issue_url: source_control.issue_url || task_issue[:url],
      pr_url: source_control.pr_url
    })
  end
    ...
    retry_index =
      ...
      |> Enum.map(fn entry ->
        status = %{
          ...
          error: sanitize_text(Map.get(entry, :error)),
