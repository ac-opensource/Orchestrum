defmodule SymphonyElixir.CLITest do
  ...
  test "returns the guardrails acknowledgement banner when the flag is missing" do
    ...
    assert banner =~ "This Orchestrum implementation is a low key engineering preview."
    assert banner =~ "Codex will run without any guardrails."
    assert banner =~ "Orchestrum is not a supported product and is presented as-is."
  ...
  test "returns startup error when app cannot start" do
    ...
    assert {:error, message} = CLI.evaluate([@ack_flag, "WORKFLOW.md"], deps)
    assert message =~ "Failed to start Orchestrum with workflow"
    assert message =~ ":boom"
  end
